...
A GUI similar to the image below will appear upon entering of the `get-timeslots` command.
<br>
...
**Caution:** Short timeslots may not display labels. You can hover over the timeslot to view its details instead.
...
| **PHONE**           | Student's phone number                           | `p/`                                            | Phone numbers may include an optional country code (e.g. +65-), followed by at least 3 digits. The digits following the country code may be seperated by hyphens (-). | `add`, `edit`                                            |
